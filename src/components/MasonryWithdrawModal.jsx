...
const tokens = [
  { id: 1, name: "LODGE", address: "0x3fB1987A692b6756b98f89321c89561f425524a3" },
...
export default function MasonryWithdrawModal( ) {
  ...
  const { config, error,refetch } = usePrepareContractWrite({
    address: '0x28B67c9A8da05304d324379883e5531F013dC485',
    abi: BoardRoomABI,
  ...
  const { config:approveConfig, error:approveError } = usePrepareContractWrite({
    address: "0x75C5a12103e8Cac4f7d79fC282112047a33F40D3",
    abi: BUSDABI,
    functionName: 'approve',
    chainId: 421613,
    args: ['0x38664595F7dD8506f86db4815dE3b9167ac6ca12','1000000000000000000000000'],
  ...
  return (
    <> {console.log(`HERE ${error}`)}  <button onClick={handleOpen} className="text-xl m-2 flex-auto rounded-lg p-0.5 px-2 bg-black hover:scale-110 text-white border-2 border-white  hover:bg-white hover:text-black hover:border-2 hover:border-black  transition duration-300 ease-in-out">
    Withdraw
  </button>
        ...
        <div className='text-base text-black'>Available { (tokens[0].name === "USDT"|| tokens[0].name === "USDC")? (Math.round(100*ethers.utils.formatUnits(balance, 6) 
                ...
                <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                  ...
                  >{isLoading &&`Waiting for approval` }
                   ...
                   {!isSuccess && !isLoading && !finished && !approveLoading&&`Withdraw $ ${ValueOrder} LODGE` }
                   {isSuccess  && finished &&`Withdraw $ ${ValueOrder} LODGE` }
